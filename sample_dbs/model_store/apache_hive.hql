/**
  ...
  This sample provides the schema and data of a database used by a store selling miniature replica models.
  The database is named model_store and has 8 tables.

  Adapted from MySQL Sample Database classicmodels V3.1 by mysqltutorial.org
  Link: http://www.mysqltutorial.org/mysql-sample-database.aspx
*/

CREATE DATABASE IF NOT EXISTS `model_store`;

/*
  Note: If you are using Hue dont run the USE statement.
  Instead create the database as above, select it in the editor and then run the remaining table statements.
*/
USE `model_store`;
...
CREATE TABLE `offices` (
  ...
  PRIMARY KEY (`office_code`) DISABLE NOVALIDATE RELY
);
...
CREATE TABLE `employees` (
  ...
  PRIMARY KEY (`employee_number`) DISABLE NOVALIDATE RELY,
  CONSTRAINT `employees_ibfk_1` FOREIGN KEY (`reports_to`) REFERENCES `employees` (`employee_number`) DISABLE NOVALIDATE,
  CONSTRAINT `employees_ibfk_2` FOREIGN KEY (`office_code`) REFERENCES `offices` (`office_code`) DISABLE NOVALIDATE
);
...
CREATE TABLE `customers` (
  ...
  PRIMARY KEY (`customer_number`) DISABLE NOVALIDATE RELY,
  CONSTRAINT `customers_ibfk_1` FOREIGN KEY (`sales_rep_employee_number`) REFERENCES `employees` (`employee_number`) DISABLE NOVALIDATE
);
...
CREATE TABLE `orders` (
  ...
  `comments` string,
  `customer_number` int NOT NULL,
  PRIMARY KEY (`order_number`) DISABLE NOVALIDATE RELY,
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`customer_number`) REFERENCES `customers` (`customer_number`) DISABLE NOVALIDATE
);
...
CREATE TABLE `productlines` (
  ...
  `image` binary,
  PRIMARY KEY (`product_line`) DISABLE NOVALIDATE RELY
);
...
CREATE TABLE `products` (
  ...
  PRIMARY KEY (`product_code`) DISABLE NOVALIDATE RELY,
  CONSTRAINT `products_ibfk_1` FOREIGN KEY (`product_line`) REFERENCES `productlines` (`product_line`) DISABLE NOVALIDATE
);
...
CREATE TABLE `orderdetails` (
  ...
  PRIMARY KEY (`order_number`,`product_code`) DISABLE NOVALIDATE RELY,
  CONSTRAINT `orderdetails_ibfk_1` FOREIGN KEY (`order_number`) REFERENCES `orders` (`order_number`) DISABLE NOVALIDATE,
  CONSTRAINT `orderdetails_ibfk_2` FOREIGN KEY (`product_code`) REFERENCES `products` (`product_code`) DISABLE NOVALIDATE
);
...
CREATE TABLE `payments` (
  ...
  PRIMARY KEY (`customer_number`,`check_number`) DISABLE NOVALIDATE RELY,
  CONSTRAINT `payments_ibfk_1` FOREIGN KEY (`customer_number`) REFERENCES `customers` (`customer_number`) DISABLE NOVALIDATE
);
